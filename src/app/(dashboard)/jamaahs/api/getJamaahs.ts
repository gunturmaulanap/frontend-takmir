...
// Get single jamaah by ID
export const getJamaah = async (id: number): Promise<Jamaah> => {
  try {
    const response = await axiosInstance.get(`/api/admin/jamaahs/${id}`);
    ...
    throw new Error(
      error instanceof Error ? error.message : "Failed to fetch jamaah"
    );
  }
};

export const getJamaahs = async (
  page: number = 1,
  limit: number = 12
): Promise<PaginatedResponse<Jamaah>> => {
  try {
    const response = await axiosInstance.get("/api/admin/jamaahs", {
      params: {
        page,
        limit,
      },
    ...
    throw new Error(
      error instanceof Error ? error.message : "Failed to fetch jamaahs"
    );
  }
};
