...
export function JamaahListPage({ onDelete }: JamaahListPageProps) {
  ...
  // Filter jamaah yang bergabung bulan ini (Client side filtering - hanya berlaku untuk page aktif)
  // Note: Idealnya statistik ini dari API dashboard terpisah
  const getJamaahBulanIni = () => {
    const now = new Date();
    const currentMonth = now.getMonth();
    const currentYear = now.getFullYear();

    return jamaahs.filter((jamaah) => {
      const createdAt = new Date(jamaah.created_at);
      return (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {isLoading ? "..." : getJamaahBulanIni().length}
              </p>
            </div>
          </div>
        </div>
