...
import { useCreateImam } from "@/hooks/useImams";
...
export default function CreateImamForm() {
  const router = useRouter();

  // ✅ Global state - Create imam mutation
  const createImamMutation = useCreateImam();
  ...
  const onSubmit = async (data: ImamFormValues) => {
    try {
      toast.loading("Menyimpan data imam...", {
        id: "create-imam",
      });

      // ✅ Create imam - validation sudah ditangani oleh schema
      await createImamMutation.mutateAsync(data);

      toast.success("Data imam berhasil ditambahkan!", {
        id: "create-imam",
      });
  ...
  return (
    <>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Tambah Imam</h1>
          <p className="text-gray-600 mt-2">
            Masukkan data imam baru untuk masjid
          </p>
        </div>
      </div>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t">
              <Button
                ...
                disabled={createImamMutation.isPending}
              >
